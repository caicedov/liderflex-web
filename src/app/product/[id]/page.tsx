...
import { Heart, Share2, Minus, Plus, Shield, RotateCcw } from "lucide-react";
...
function ProductDetailContent({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addToCart = () => {
    if (product.availableSizes && !selectedSize) {
      setModalData({ open: true, message: "Por favor selecciona una medida" });
      return;
    }
    ...
    setModalData({
      open: true,
      message: `Se añadieron ${quantity} producto(s) al carrito de cotización.`,
    });
  };
  ...
  return (
    <>
      ...
      <SuccessModal
        ...
        onClose={() => setModalData({ open: false, message: "" })}
